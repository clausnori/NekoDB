...
class RecordBuffer:
    ...
    HEADER_FORMAT = "!Q"  # 8-byte length prefix
...
class DB:
  ...
  def _create_shard(self, table: TableMetadata, start_key: Any, end_key: Any) -> TableShard:
  ...
  def _find_shard(self, table: TableMetadata, key: Any) -> Optional[TableShard]:
      ...
      for shard in table.shards:
          if (shard.start_key is None or key >= shard.start_key) and            (shard.end_key is None or key <= shard.end_key):
              return shard
      return None
  ...
  def _build_string_index_shard(self, table: TableMetadata, field: str, 
                              shard: TableShard) -> Dict[str, List[int]]:
  ...
  def _rebalance_shards(self, table: TableMetadata):
      """Rebalance shards if needed"""
      shards_to_split = [
          shard for shard in table.shards 
          if shard.record_count > self.max_shard_size
      ]
      
      if not shards_to_split:
          return
          
      for shard in shards_to_split:
          self._split_shard(table, shard)
  
  def _split_shard(self, table: TableMetadata, shard: TableShard):
      """Split shard into two"""
      records = []
      
      # Read all records from shard
      with RecordBuffer(shard.file_path) as buf:
          while record_data := buf.read_record():
              record = CompressedRecord.decompress(record_data)
      ...
      # Find split point
      mid = len(records) // 2
      split_key = records[mid].get(next(col.name for col in table.columns.values() 
                                      if col.shard_key))
                                      
      # Create new shards
      new_shard1 = self._create_shard(table, shard.start_key, split_key)
      new_shard2 = self._create_shard(table, split_key, shard.end_key)
      
      # Distribute records
      with RecordBuffer(new_shard1.file_path, 'wb') as buf1,          RecordBuffer(new_shard2.file_path, 'wb') as buf2:
          
          for record in records:
              shard_key = record.get(next(col.name for col in table.columns.values() 
                                        if col.shard_key))
              if shard_key <= split_key:
                  buf1.write_record(CompressedRecord.compress(record))
                  new_shard1.record_count += 1
              else:
                  buf2.write_record(CompressedRecord.compress(record))
                  new_shard2.record_count += 1
                  
      # Remove old shard
      table.shards.remove(shard)
      Path(shard.file_path).unlink()
  
  def insert(self, table_name: str, record: Dict[str, Any]) -> None:
      """Insert record with automatic sharding"""
      with self._table_lock(table_name):
          ...
          shard_key_field = next(col.name for col in table.columns.values() 
                               if col.shard_key)
          ...
          if shard.record_count > self.max_shard_size:
              self._rebalance_shards(table)
              
          self._save_metadata()
  
  def _parallel_search_shard(self, shard: TableShard, conditions: List[QueryCondition],
                           projection: Set[str] = None) -> List[Dict[str, Any]]:
      ...
      with RecordBuffer(shard.file_path) as buf:
          while record_data := buf.read_record():
              ...
              if all(condition.evaluate(record) for condition in conditions):
                  if projection:
                      results.append({k: v for k, v in record.items() if k in projection})
                  else:
  ...
  def find(self, table_name: str, conditions: List[QueryCondition] = None,
      ...
      """Enhanced parallel search with projection and optimizations"""
      if table_name not in self.tables:
          raise ValueError(f"Table {table_name} not found")
      ...
      # Try using string index for startswith conditions
      string_conditions = [
  ...
  def _validate_record(self, table: TableMetadata, record: Dict[str, Any]) -> Dict[str, Any]:
      ...
      for field, column in table.columns.items():
          ...
          try:
              ...
              if not self._validate_types(converted, column.types):
                  raise ValueError(f"Value {value} does not match types {column.types}")
  ...
  def _update_shard(self, table: TableMetadata, shard: TableShard,conditions: List[QueryCondition], 
                   updates: Dict[str, Any]) -> int:
      ...
      with self._get_shard_lock(table.name, shard.file_path):
          with RecordBuffer(shard.file_path) as read_buf,             RecordBuffer(str(temp_file), 'wb') as write_buf:
  ...
  def _delete_shard(self, table: TableMetadata, shard: TableShard,
  ...
  def _remove_from_indexes(self, table: TableMetadata, record: Dict[str, Any],position: int) -> None:
  ...
  def _validate_reference(self, table_name: str, column: Column, value: Any) -> None:
      ...
      if not self.exists(ref_table, conditions):
          raise ValueError(f"Referenced value {value} not found in {ref_table}.{ref_field}")
  ...
  def count(self, table_name: str, conditions: List[QueryCondition] = None) -> int:
      ...
      for shard in self.tables[table_name].shards:
          futures.append(
              self.worker_pool.submit(
                  self._count_shard, shard, conditions
              )
          )
